# voca app — CLAUDE.md (Claude Code)

Claude Code 작업의 1차 진입 문서. Codex는 `AGENTS.md`를 사용한다. 두 파일은 도구 framing만 다르고 섹션·순서·규칙은 동일하게 유지한다. 새 규칙은 양쪽 모두 갱신한다.

## Claude Code 환경 운용

- 파일 탐색은 `Glob` / `Grep` 우선, 전 범위 탐색이 필요할 때만 Agent + Explore subagent 사용.
- 파일 수정은 `Edit` / `Write`. shell heredoc·`cat <<EOF` 으로 파일을 만들지 않는다.
...
- commit message에 `Co-Authored-By: Claude...` 트레일러를 자동 삽입하는 기본 동작은 끄거나 명시적 요청 때만 사용.
- merge/push는 사용자가 명시적으로 요청할 때만.
- 사용자 시지에 부합하는 superpowers 스킬(brainstorming, TDD, debugging 등)은 작업 성격이 명확히 일치할 때만 호출. trivial 요청에 강제 적용 X (전역 지침 기준).
...
전역 `~/.claude/CLAUDE.md` "임용 관련 작업: 한국어 용어 규칙"을 따른다. 일반 학습 어휘에 한국어 표현이 자연스러우면 그대로 사용. 임용 브랜치(`FUTURE_WORK.md`) 작업에 들어가면 합격자 노트/강사 교재/기출 원문에 등장한 한국어 표현만 사용.
...
기본은 묻지 말고 알아서 처리 (auto-memory 기준). 다음만 묻는다.
